...
invoking the build pipeline, `alr-builder` ensures `lambda.env`
reflects the current `lambda.yaml`, when that workflow is in use.
The build requires a configured function name and will abort if
`FUNCTION_NAME` is unset.
...
The build pipeline ultimately consumes `lambda.env`, a flat
`KEY = value` file read by `Makefile.mk`.  Project configuration can
either be maintained directly in that file or expressed in
`lambda.yaml`, from which `alr-builder` generates `lambda.env`.
...
`META.json`.
    ...
    In a project generated by `CPAN::Maker::Bootstrapper`, subsequent
    handler changes can normally be rebuilt by running `make`. By
    default, `make` scans your files for new dependencies, updates
    `cpanfile`, lints your code (`perlcritic`, `perltidy` if
    available), and builds the new tarball. Use `make quick` to skip
    linting.
...
The `check` command verifies the following baseline permissions via
`SimulatePrincipalPolicy`.  They cover the common build-and-deploy path;
