...
$(CACHE_DIR)/lambda-function-url:  $(CACHE_DIR)/lambda-function-url-permission \
	...
	url="$$(alr-helper --report-step $@ get-function-url-config $(FUNCTION_NAME) 2>&1 || true)"; \
	if echo "$$url" | grep -q 'ResourceNotFoundException'; then \
	    url="$$(alr-helper --report-step $@ create-function-url-config $(FUNCTION_NAME) $(INVOKE_MODE) | dnk FunctionURL)"; \
