...
$(CACHE_DIR)/lambda-eventbridge-rule: ## create EventBridge schedule rule | $(CACHE_DIR)
...
$(CACHE_DIR)/lambda-eventbridge-permission: ## grant EventBridge permission to invoke Lambda \
    $(CACHE_DIR)/lambda-function \
    $(CACHE_DIR)/lambda-eventbridge-rule | $(CACHE_DIR)
...
$(CACHE_DIR)/lambda-eventbridge-trigger: ## add Lambda as EventBridge rule target \
    $(CACHE_DIR)/lambda-eventbridge-permission | $(CACHE_DIR)
	$(NO_ECHO)trigger="$$(alr-helper put-lambda-target $(FUNCTION_NAME) $(RULE_NAME) lambda-handler)"; \
	test -e $@ || echo "$$trigger" > $@; \
...
delete-eventbridge-rule: ## remove targets and delete EventBridge rule
	$(NO_ECHO)alr-helper remove-targets $(RULE_NAME) lambda-handler; \
